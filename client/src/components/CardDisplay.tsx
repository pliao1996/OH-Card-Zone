...
export function CardDisplay({
  ...
  const isImageContent =
    ...
    (card.content.match(/\.(jpg|jpeg|png|webp|gif)$/i) ||
      card.content.includes("_"));

  // For large size, show image directly without animation
  if (size === "lg") {
    return (
      <div
      ...
      >
        {!isRevealed ? (
          ...
          <div className="text-center w-full flex flex-col items-center justify-center p-6">
            <div className="border-2 border-primary/10 rounded-lg p-4 flex items-center justify-center">
              <span className="font-display font-bold text-2xl text-primary tracking-wider uppercase">
                {card.content}
              </span>
            </div>
            <span className="mt-4 text-xs text-muted-foreground/50 font-mono">
              #{card.number}
            </span>
          </div>
        ) : (
  ...
  // Regular animated card for small/medium sizes
  return (
    <div
      className={cn(
        ...
        <div className="absolute inset-0 backface-hidden w-full h-full shadow-2xl rounded-xl overflow-hidden bg-white border border-border">
          ...
          <div className="absolute inset-0 flex items-center justify-center z-10 pl-[14.5%] pr-[14%] pt-[16.5%] pb-[16%]">
            <div className="w-full h-full rounded-sm overflow-hidden shadow-inner border border-border/10">
              <img
                src={
                  imageCard.content.startsWith("http")
                    ? imageCard.content
                    : `/images/${imageCard.content}`
                }
                alt={imageCard.title}
                className="w-full h-full object-cover"
              />
